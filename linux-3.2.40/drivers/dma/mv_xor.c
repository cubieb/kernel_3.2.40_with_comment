				   ...
				   struct mv_xor_desc_slot *sw_desc)
{
	...
	if (sw_desc->type == DMA_MEMSET) {
		...
		//sw_desc->async_tx.phys:该slot在内存中的物理地址
...
mv_xor_clean_completed_slots(struct mv_xor_chan *mv_chan)
{
	...
	list_for_each_entry_safe(iter, _iter, &mv_chan->completed_slots,
        ...
        //如果已经进行了ack处理，将slot从completed_slots中删除
		if (async_tx_test_ack(&iter->async_tx)) {
			list_del(&iter->completed_node);
			mv_xor_free_slots(mv_chan, iter);  /*标记为未使用*/
		}
	}
	return 0;
}
...
static void __mv_xor_slot_cleanup(struct mv_xor_chan *mv_chan)
{
	...
	int busy = mv_chan_is_busy(mv_chan); /* dma is busy ?? */
...
{
	struct mv_xor_desc_slot *sw_desc = to_mv_xor_slot(tx);  /* 找到所属的slot */
	struct mv_xor_chan *mv_chan = to_mv_xor_chan(tx->chan); /* 找到所属的channel */
	struct mv_xor_desc_slot *old_chain_tail;

	dma_cookie_t cookie;
	//不放到spin_lock_bh里面不会有问题吗???????
	int new_hw_chain = 1;
	...
	if (list_empty(&mv_chan->chain)) {
		...
		//找到最后一个descriptor list
		old_chain_tail = list_entry(mv_chan->chain.prev,
					    struct mv_xor_desc_slot,
					    chain_node);
		//将新加入的descriptor list加到mv_chan->chain的最后，也就是位于old_chain_tail的后面
		list_add_tail(&sw_desc->chain_node, &mv_chan->chain);
		...
		if (!mv_chan_is_busy(mv_chan)) { //not busy
			//读出当前descriptor的值
			u32 current_desc = mv_chan_get_current_desc(mv_chan);
			/*
...
static int mv_xor_alloc_chan_resources(struct dma_chan *chan)
{
	...
	while (idx < num_descs_in_pool) {
		...
		//将分配的slot链入channel的链表
		list_add_tail(&slot->slot_node, &mv_chan->all_slots);
		spin_unlock_bh(&mv_chan->lock);
	}

	if (mv_chan->slots_allocated && !mv_chan->last_used) {
		//最开始将last_used指定为第一个slot
		...
		size_t len, unsigned long flags)
{
	...
	//software descriptor, slot
	sw_desc = mv_xor_alloc_slots(mv_chan);  /*得到一个slot*/
	if (sw_desc) {
		//XOR engine 模式
...
static int __devinit mv_xor_probe(struct platform_device *pdev)
{
	int ret = 0;
	int irq;
	struct mv_xor_device *adev;  //adev: a device
	struct mv_xor_chan *mv_chan;
	...
	dma_dev = &adev->common;  //嵌入的dma device
	...
	adev->dma_desc_pool_virt = dma_alloc_writecombine(&pdev->dev,
							  plat_data->pool_size,   //要分配的大小
							  &adev->dma_desc_pool,  //物理地址
							  GFP_KERNEL);
	...
	//该通道的所属，属于哪个devide
	mv_chan->device = adev;
	mv_chan->idx = plat_data->hw_id;  //channel number
	//寄存器基地址
			 ...
			 struct mbus_dram_target_info *dram)
{
	...
	for (i = 0; i < dram->num_cs; i++) {
		...
		win_enable |= (1 << i);  /* 使能的window */
		win_enable |= 3 << (16 + (2 * i));  /* 访问权限, 3: read and write */
	}
