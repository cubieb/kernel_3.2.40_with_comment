...
struct mv_xor_chan {
	...
	void __iomem		*mmr_base;  /*寄存器基地址*/
	unsigned int		idx; /*xor通道编号*/
	enum dma_transaction_type	current_type;
	struct list_head	chain;  /* 将要被传输的slot */
	struct list_head	completed_slots; /*硬件已经完成操作，但是软件还没有进行ack处理*/
	struct mv_xor_device	*device; /*parent*/
...
struct mv_xor_desc_slot {
	...
	void			*hw_desc;
	/* 标记该slot是否正在被使用 */
	u16			slots_per_op; /* 一次操作需要的slot数 !!!!! 应该不是这个意思*/
	u16			idx;
	u16			unmap_src_cnt;
	u32			value;
	size_t			unmap_len;
