...
static struct resource axp_i2c_0_resources[] = {
	{
		...
		.end    = INTER_REGS_PHYS_BASE + MV_TWSI_SLAVE_REGS_OFFSET(0) + 0x100 - 1,
...
static struct resource axp_i2c_1_resources[] = {
	{
		...
		.end    = INTER_REGS_PHYS_BASE + MV_TWSI_SLAVE_REGS_OFFSET(1) + 0x100 - 1,
		.flags  = IORESOURCE_MEM,
	},
	{
		.name   = "i2c irq",
...
static void __init armadaxp_xor0_init(void)
{
	 ...
	 */
	 //channel00只有两种功能，memcpy and xor
    ...
    //channel00只有三种功能，memcpy memset and xor
