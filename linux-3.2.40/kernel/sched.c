...
struct rt_rq {
	struct rt_prio_array active;  /* bitmap和优先级队列 */
...
struct rq {
	 ...
	 */
	unsigned long nr_running; /* 可运行进程的数量 */
	...
	struct cfs_rq cfs;  /* 均衡调度使用的运行队列 */
	struct rt_rq rt;    /* 实时调度使用的运行队列 */
	...
	int cpu;  /* 该rq所属的cpu */
	int online;  /* 置位表示有task在对应的cpu上运行 */
...
/* 为该task设置相应的标志位，使调度器可以调度该task运行 */
static void resched_task(struct task_struct *p)
{
	...
	cpu = task_cpu(p);
	/* 如果该线程正好在当前的cpu上，直接返回，否则发送IPI中断 */
	...
	smp_mb();
	if (!tsk_is_polling(p))
		smp_send_reschedule(cpu);  /* 发送IPI中断，指定的cpu可以调度该task */
}
...
static void ttwu_activate(struct rq *rq, struct task_struct *p, int en_flags)
{
	...
	if (p->flags & PF_WQ_WORKER)  /* 如果当前进程是一个workqueue线程 */
...
{
    //wakeup的进程并不是添加到runqueue上去，而是被加到该进程所在cpu的wake_list上去
...
try_to_wake_up(struct task_struct *p, unsigned int state, int wake_flags)
{
	...
	if (p->sched_class->task_waking)  /* rt 调度没有实现该函数 */
...
need_resched:
	...
	raw_spin_lock_irq(&rq->lock);  /* 不能被中断 */
...
#ifdef CONFIG_SMP
	init_defrootdomain();  /* 初始化root domain */
#endif
